...
```mermaid
graph LR
    A1["입력 문장"] -- "Self-Attention" --> B1["문맥적 관계 파악"]
    B1 -- "Feed Forward" --> C1["인코딩된 표현"]
    C1 -- "Decoder" --> D1["결과 생성"]
```

**정의**: 순차적 연산이 필요한 RNN의 한계를 극복하고, **Self-Attention** 메커니즘을 통해 문장 내 모든 단어 간의 관계를 병렬적으로 처리하는 혁신적인 신경망 아키텍처  
